...
class LinearExpertsDistribution(TorchDistributionWrapper):
    ...
    def __init__(self, inputs, model):
        ...
        legal = LinearExpertsDistribution.build_legal_masks()
    ...
    #     expert_size = LinearLogitHead.num_outputs()
    #     start = self.num_experts + selected_expert * expert_size
    ...
    def expert_logits(self, selected_expert):
        # inputs: [B, E + E*S + 1]
        #   E = self.num_experts
        #   S = expert_size = LinearLogitHead.num_outputs()
    ...
    def sample(self, deterministic=False):
        ...
        self._action_logp += mode_dist.log_prob(mode)
    ...
    def entropy(self):
        ...
        for i in range(self.num_experts):
            ...
            mode_logits = self.mode_logits(i)
            mode_logits = self.adjust_mode_logits(
                mode_logits, self.cannot_discard_flags()
            )
            mode_dist = Categorical(logits=mode_logits)
            self._entropy += mode_dist.entropy() * expert_prob

            # Add the entropy of the cards
            for mode in range(2):
                cards_entropy = self.cards_entropy(i, mode)
                self._entropy += cards_entropy * mode_dist.probs[:, mode] * expert_prob
...
LinearExpertsDist = LinearExpertsDistribution
