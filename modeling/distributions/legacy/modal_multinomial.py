...
class ModalMultinomialDistribution(TorchDistributionWrapper):
    ...
    def sample_cards(self, mode, deterministic):
        ...
        if deterministic:
            ...
            indices = torch.multinomial(probs, num_samples=5, replacement=False)
            cards[torch.arange(BATCH).unsqueeze(1), indices] = 1
            # print("stochastic", cards.shape)
        # print(cards.shape)
        return cards
    ...
    def entropy(self):
        ...
        for mode in range(2):
            ...
            cards_entropy += mode_probs[:, mode] * self.cards_entropy(mode)
...
ModalMultinomialDist = ModalMultinomialDistribution
