...
class ParametricSequentialBalatroPlayHandModel(TorchModelV2, nn.Module):
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        starting_embeddings[52, 18] = 1.0
        starting_embeddings[53, 19] = 1.0
        ...
        class _ActionModel(nn.Module):
            def __init__(self):
                ...
                self.h2 = nn.Linear(action_module_size, action_module_size)
                # self.intent_layer = nn.Linear(action_module_size, a_vec_size)
                self.intent_layer = nn.Linear(action_module_size, card_embedding_size)
            ...
            def forward(self, context, actions):
                ...
                intent = self.intent_layer(h2)
                intent = torch.unsqueeze(intent, 1)
                intent = self.tanh(intent)
